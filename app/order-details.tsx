...
const ORDERS: Record<string, {
  ...
  "1": {
    ...
    timeline: [
      { label: "Delivered", time: "Oct 24, 4:15 PM", note: "Received by Customer" },
      { label: "Out for Delivery", time: "Oct 24, 3:45 PM", note: "Courier is nearby" },
      { label: "Order Shipped", time: "Oct 24, 3:00 PM", note: "Leaves TechHub Store" },
      { label: "Order Confirmed", time: "Oct 24, 2:35 PM", note: "Payment Verified" },
    ],
    deliveryLocation: {
      name: "UR CST Main Gate",
      address: "University of Rwanda, College of Science and Technology, Nyarugenge",
    },
  },
  "2": {
    orderNumber: "#1244",
    ...
    timeline: [
      { label: "Delivered", time: "Oct 22, 2:00 PM", note: "Received by Customer" },
      { label: "Out for Delivery", time: "Oct 22, 1:45 PM", note: "Courier is nearby" },
      { label: "Order Shipped", time: "Oct 22, 1:30 PM", note: "Leaves Campus Bites" },
      { label: "Order Confirmed", time: "Oct 22, 1:12 PM", note: "Payment Verified" },
    ],
    deliveryLocation: {
      name: "UR CST Main Gate",
      address: "University of Rwanda, College of Science and Technology, Nyarugenge",
    ...
    <SafeAreaView className="flex-1 bg-slate-50" edges={["top"]}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1">
        ...
        <View className="mx-4 mt-4 bg-white rounded-3xl p-4 border border-slate-100">
          ...
          <Text className="text-xl font-bold text-slate-900">Order {order.orderNumber}</Text>
          <Text className="text-xs text-slate-500 mt-0.5 mb-4">{order.placedAt}</Text>
          ...
          <View className="flex-row items-center bg-slate-50 rounded-2xl p-3">
            ...
            <View className="flex-1 ml-3">
              ...
              <Text className="text-xs text-slate-500 mt-0.5">{order.product.store}</Text>
              <Text className="text-xs text-slate-400 mt-0.5">Qty: {order.product.qty}</Text>
            </View>
          </View>
        </View>
            ...
            return (
              <View key={index} className="flex-row">
                ...
                <View className="pb-5 flex-1">
                  <Text className="text-sm font-bold text-slate-900">{step.label}</Text>
        ...
        <TouchableOpacity className="mx-4 mt-4 bg-white rounded-2xl py-4 flex-row items-center justify-center border border-slate-100">
          ...
          <Text className="text-sm font-bold text-slate-900 ml-2">Feedback</Text>
        </TouchableOpacity>
      ...
      <View className="absolute bottom-0 left-0 right-0 bg-white border-t border-slate-100 px-4 py-4 pb-8">
        <TouchableOpacity className="bg-primary flex-row items-center justify-center py-4 rounded-2xl">
          ...
          <Text className="text-white font-bold text-base ml-2">Reorder Items</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
